...
const createStartIconHtml = (name = "Location name") => ReactDOMServer.renderToString(
...
const createDestIconHtml = (name = "Location name") => ReactDOMServer.renderToString(
  <div className="relative flex items-center justify-center">
    <div className="absolute w-12 h-12 rounded-full bg-pink-500/40 blur-md animate-pulse" />
    <div className="absolute -top-12 bg-white text-gray-900 px-3 py-1 rounded-2xl shadow-xl flex flex-col items-start whitespace-nowrap border border-pink-100">
...
// agar halaman lain tidak error
export const startIcon = new Proxy(
  (name = "Location name") => L.divIcon({
    className: "custom-start-marker",
    html: createStartIconHtml(name),
    iconSize: [40, 40],
    iconAnchor: [20, 20],
  }),
  {
    get(target, prop) {
      const defaultInstance = L.divIcon({
        className: "custom-start-marker",
        html: createStartIconHtml("Location name"),
        iconSize: [40, 40],
        iconAnchor: [20, 20],
      });
...
export const destinationIcon = new Proxy(
  (name = "Location name") => L.divIcon({
    className: "custom-dest-marker",
    html: createDestIconHtml(name),
    iconSize: [40, 40],
    iconAnchor: [20, 20],
  }),
  {
    get(target, prop) {
      const defaultInstance = L.divIcon({
        className: "custom-dest-marker",
        html: createDestIconHtml("Location name"),
        iconSize: [40, 40],
        iconAnchor: [20, 20],
      });
      return defaultInstance[prop];
