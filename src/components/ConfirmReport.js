...
export default function ConfirmReport({ isOpen, onClose, onConfirm, formData, previewUrls, isLoading }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-[#272133] border border-white/10 rounded-[30px] p-6 sm:p-8 text-white shadow-2xl flex flex-col items-center">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-center mb-1">
          Are you sure to submit this report?
        </h2>
        <p className="text-xs sm:text-sm text-gray-300 text-center mb-6">
          Make sure all of the informations are correct.
        </p>

        {/* preview Card dalam Popup */}
        <div className="w-full bg-[#1e1728] border border-white/10 rounded-2xl p-5 mb-6 shadow-inner">
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full border border-pink-500/50 bg-pink-500/10 text-pink-300">
        ...
        <div className="w-full flex flex-col gap-3">
          <button 
            ...
            onClick={onConfirm}
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-pink-400 to-pink-600 text-white py-3.5 rounded-full font-semibold text-sm transition-all shadow-md hover:opacity-95 flex items-center justify-center gap-2 cursor-pointer">
          ...
          <button 
            type="button"
            onClick={onClose}
            className="w-full bg-transparent border border-white/30 text-white py-3 rounded-full font-semibold text-sm transition-all hover:bg-white/10 cursor-pointer">
            Cancel
          </button>
        </div>
