...
export default function AnonymousReportPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // TEMPAT UNTUK FETCH API NANTI
      // buka comment fetch dan memakai dummy API (jsonplaceholder) biar proses loading & submit bisa dites jalan
      ...
      // ganti setTimeout karena fetch ke dummy API di atas sudah berjalan asinkron
      alert("Success!");

      // mengosongkan form kembali setelah berhasil submit biar rapi
      ...
      console.error(error);
      // alert error jika internet mati atau API gagal
  ...
  return (
    <div className="min-h-screen w-full bg-white text-gray-800 font-sanmax-w-5xl mx-auto w-full pt-28 sm:pt-32 pb-20 px-4 sm:px-8">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8 w-full">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Input"
              className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-400/50 transition-all placeholder-gray-400" />
          </div>
          ...
          <div className="flex flex-col gap-2 relative" ref={locationRef}>
            ...
            {/* area klik untuk ngebuka/nutup dropdown + icon panah */}
            <div 
            ...
            {/* menu dropdown (kolom search + daftar lokasi) yang muncul pas di-klik */}
            {isLocationOpen && (
                    ...
                    .map((loc, index) => (
                      <div 
                        ...
                        onClick={() => {
                          ...
                          setIsLocationOpen(false);
                          setLocationSearch(""); // Reset search pas udah milih
                        }}
        ...
        <div className="flex flex-col gap-2 w-full border-t border-gray-100 pt-6">
          ...
          <textarea name="description" rows={5} value={formData.description} onChange={handleChange} placeholder="Input description of the incident..."
            className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-400/50 transition-all placeholder-gray-400 resize-none" />
        </div>
        ...
        <div className="w-full flex justify-center pt-8 pb-10">
          <button type="submit" disabled={isLoading}
            className="w-full sm:w-auto bg-gradient-to-r from-pink-400 to-pink-500 hover:from-pink-500 hover:to-pink-600 text-white px-8 py-3.5 rounded-full font-semibold text-sm transition-all flex items-center justify-center gap-2 shadow-sm disabled:opacity-70">
